  ...
  \internal
    ...
    \subsection sdhlibrary_cpp_sdh_cpp_details SVN related, detailed file specific information:
      ...
      \par SVN file revision:
        $Id: sdh.cpp 11045 2013-11-27 15:12:49Z Osswald2 $
...
char const* cSDH::GetFirmwareReleaseRecommended( void )
{
    return FIRMWARE_RELEASE_RECOMMENDED;
}
//----------------------------------------------------------------------
...
throw (cSDHLibraryException*)
{
    ...
    if ( ! strcmp( what, "date" ) || ! strcmp( what, "date-library" ) )
        return PROJECT_DATE;
    if ( ! strcmp( what, "release-firmware-recommended" ) )
        return GetFirmwareReleaseRecommended();
...
void cSDH::UpdateSettingsFromSDH()
{
    ...
    // GetAxisLimitVelocity() reports velocities in external units,
    // but f_max_velocity_v should store in internal units, so convert:
    std::vector<double>::iterator vi;
    for( vi = f_max_velocity_v.begin();
         vi != f_max_velocity_v.end();
         vi++ )
    {
        *vi = uc_angular_velocity->ToInternal( *vi );
    }
    ...
    // GetAxisLimitAcceleration() reports accelerations in external units,
    // but f_max_acceleration_v should store in internal units, so convert:
    for( vi = f_max_acceleration_v.begin();
         vi != f_max_acceleration_v.end();
         vi++ )
    {
        *vi = uc_angular_acceleration->ToInternal( *vi );
    }
